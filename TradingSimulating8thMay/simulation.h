...
public:
    ...
    map<string,long> executer(Bourse& bourse,Trader& trader)
    {
       ...
       auto actionsAujourdhui = bourse.getActionsDisponiblesParDate(d1);
       ...
       while(d<=datefin)
       {
           ...
           {
               triplet trp;
               trp=trader.prendreDecision(bourse,portefeuille,d);
               ...
               if(choix==0)
               {
                   if(r>=0)
                   {
                       ...
                       string s=bourse.getActionsDisponiblesParDate(d).at(r);
               ...
               if(choix==1)
               {
                   if(r>=0)
                   {
                       ...
                       portefeuille.deposermontant(bourse.dernierprix(portefeuille.Titres.at(r).getAction(),d)*x);
                       cout<<" Le Traideur a vendu l'action ";
                       cout<<portefeuille.Titres.at(r).getAction();
                       cout<<" d'une quantite ";
                       cout<<x;
                       cout<<"Le montant depose est  ";
                       cout<<bourse.dernierprix(portefeuille.Titres.at(r).getAction(),d)*x<<endl;
                       stats["NB_VENTE"]++;
        ...
        while(portefeuille.Titres.size()!=0)
        {
            ...
            double x=bourse.dernierprix(portefeuille.Titres.at(0).getAction(),datefin);
