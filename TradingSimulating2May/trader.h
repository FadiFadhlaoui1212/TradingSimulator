...
public:
    ...
    if(choix==1)
    {
        cout<<"Voici les actions disponibles de la date d'aujourd'hui, tapez le numero de l'action que vous voulez achetez"<<endl;
        for(unsigned int i=0;i<boursevecteur.PrixJournaliersParDate(date).size();i++)
        {
            cout<<"numero"<<i+1<<"/";
            cout<<boursevecteur.PrixJournaliersParDate(date).at(i).getaction();
            cout<<" prix :"<<boursevecteur.PrixJournaliersParDate(date).at(i).getprix()<<"|";
        }
        do
        {
          cin>>num;
